...
export async function POST(request: Request) {
  ...
  if (action === "setStatus") {
    ...
    await prisma.document.updateMany({ where: { id: { in: ids } }, data: { status } });
  ...
  if (action === "addTag") {
    ...
    const docs = await prisma.document.findMany({ where: { id: { in: ids } }, select: { id: true, tags: true } });
    await Promise.all(
      docs.map((doc) => {
        const next = doc.tags.includes(tag) ? doc.tags : [...doc.tags, tag];
        return prisma.document.update({ where: { id: doc.id }, data: { tags: next } });
      })
    );
    return NextResponse.json({ ok: true, updated: ids.length });
  }
